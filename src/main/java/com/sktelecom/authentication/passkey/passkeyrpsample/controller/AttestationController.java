...
public class AttestationController {
	...
	AttestationOptionsServerResponse registrationChallenge(@Valid @RequestBody AttestationOptionsServerRequest request,
			HttpServletResponse httpServletResponse) {
		log.info("AttestationController.registrationChallenge()====================");
		log.info("==> request = " + request);
		log.info("==> request.getDisplayName() = " + request.getDisplayName());
		log.info("==> request.getUsername() = " + request.getUsername());
		log.info("==> request.getAttestation() = " + request.getAttestation());
		log.info("==> request.getAuthenticatorSelection() = " + request.getAuthenticatorSelection());
		log.info("==> request.getAuthenticatorSelection().getAuthenticatorAttachment() = " + request.getAuthenticatorSelection().getAuthenticatorAttachment());
		log.info("==> request.getAuthenticatorSelection().getResidentKey() = " + request.getAuthenticatorSelection().getResidentKey());
		log.info("==> request.getAuthenticatorSelection().isRequireResidentKey() = " + request.getAuthenticatorSelection().isRequireResidentKey());
		log.info("==> request.getAuthenticatorSelection().getUserVerification() = " + request.getAuthenticatorSelection().getUserVerification());
		log.info("==> request.getExtensions() = " + request.getExtensions());
		log.info("==> httpServletResponse = " + httpServletResponse.toString());
		
		WebAuthnServerResponse<AttestationOptions> optionsResponse = attestationService.getOptions(request);		
		log.info("optionsResponse = ", optionsResponse);		
		
		AttestationOptions options = optionsResponse.getOptions();
		log.info("options = ", options);
		log.info("optionsResponse.getSessionId() = ", optionsResponse.getSessionId());
	 ...
	 * 
	 * @param request             parameters for registration options, you may use
	 *                            your own model, most of the cases, such values are
	 *                            populated in the backend. Username and user id
	 *                            values should be obtained from the authenticated
	 *                            session. For simplicity, this sample application
	 *                            generates the user id with username. In general,
	 *                            the services might have their own user identifier.
	 *                            The user id generation for the sample application
	 *                            is handled during model mapping. Check this
	 *                            {@link AttestationOptionsServerRequestMapper#toWebauthnUserDto(AttestationOptionsServerRequestLv3 rpServer)
	 *                            model mapper }.
	...
	AttestationOptionsServerResponse registrationChallengeLv3(
		...
		// You might have different way to manage the WebAuthn registration session
		// (transaction)
